//
//  AutocompleteSearchTextField.swift
//  DiabetesApp
//
...
open class AutocompleteSearchTextField: UITextField {
    ...
    override open func willMove(toSuperview newSuperview: UIView?) {
        ...
        self.addTarget(self, action: #selector(AutocompleteSearchTextField.textFieldDidChange), for: .editingChanged)
        self.addTarget(self, action: #selector(AutocompleteSearchTextField.textFieldDidBeginEditing), for: .editingDidBegin)
        self.addTarget(self, action: #selector(AutocompleteSearchTextField.textFieldDidEndEditing), for: .editingDidEnd)
        self.addTarget(self, action: #selector(AutocompleteSearchTextField.textFieldDidEndEditingOnExit), for: .editingDidEndOnExit)
        
        NotificationCenter.default.addObserver(self, selector: #selector(AutocompleteSearchTextField.keyboardWillShow(_:)), name: NSNotification.Name.UIKeyboardWillShow, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(AutocompleteSearchTextField.keyboardWillHide(_:)), name: NSNotification.Name.UIKeyboardWillHide, object: nil)
    }
    ...
    fileprivate func redrawSearchTableView() {
        ...
        if let tableView = tableView {
            ...
            if self.direction == .down {
                ...
                var tableViewFrame = CGRect(x: 0, y: 0, width: frame.size.width - 4, height: tableHeight>150 ? 150 :tableHeight)
                ...
                tableView.frame = CGRect(x: frame.origin.x + 2, y: (frame.origin.y - tableHeight + positionGap), width: frame.size.width - 4, height: tableHeight>150 ? 150 :tableHeight)
    ...
    open func textFieldDidChange() {
        ...
        timer?.invalidate()
        timer = Timer.scheduledTimer(timeInterval: 0.8, target: self, selector: #selector(AutocompleteSearchTextField.typingDidStop), userInfo: self, repeats: false)
...
extension AutocompleteSearchTextField: UITableViewDelegate, UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        var cell = tableView.dequeueReusableCell(withIdentifier: AutocompleteSearchTextField.cellIdentifier)
        
        if cell == nil {
            cell = UITableViewCell(style: .subtitle, reuseIdentifier: AutocompleteSearchTextField.cellIdentifier)
        }
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let userInfo = ["medicationindex":(indexPath as NSIndexPath).row,"medicationname": filteredResults[(indexPath as NSIndexPath).row].title] as [String: Any]
        NotificationCenter.default.post(name: NSNotification.Name(rawValue: Notifications.selectMedicationNotification),
                                        object: nil,
                                        userInfo: userInfo)
        clearResults()
        tableView.reloadData()
...
public struct SearchTextFieldTheme {
    ...
    public static func lightTheme() -> SearchTextFieldTheme {
        return SearchTextFieldTheme(cellHeight: 30, bgColor: UIColor (red: 1, green: 1, blue: 1, alpha: 1.0), borderColor: UIColor (red: 0.9, green: 0.9, blue: 0.9, alpha: 1.0), separatorColor: UIColor.clear, font: Fonts.noOfDaysFont, fontColor: UIColor.black)
    }
