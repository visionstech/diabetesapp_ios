...
- (void)layoutSubviews
{
    ...
    self.titleLabel.frame = CGRectMake( self.contentView.bounds.origin.x,  self.contentView.bounds.origin.y, 375,  self.contentView.bounds.size.height);
    
    if (self.header.scrollDirection == UICollectionViewScrollDirectionHorizontal) {
        CGFloat position = [self.contentView convertPoint:CGPointMake(100, CGRectGetMidY(self.contentView.bounds)) toView:self.header].x;
        CGFloat center = 100;
